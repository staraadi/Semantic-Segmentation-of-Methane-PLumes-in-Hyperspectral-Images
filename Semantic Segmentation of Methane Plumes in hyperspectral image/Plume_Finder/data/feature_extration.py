...
def use_pretrained_model_b1to6_b8(inB1,inB2,inB3,inB4,inB5,inB6,outB8, 
    ...
    if model_preloaded is None:
        ...
        with fs.open(f"gs://Plume_Finder/experiments/{experiment_path}/.hydra/config.yaml", "r") as f:
            config_model = omegaconf.OmegaConf.load(f)

        config = omegaconf.OmegaConf.merge(config, config_model)
        config.dataloader.batch_size = 1
        
        from Plume_Finder.models.model_module_regression import ModelModuleRegression
        checkpoint_path = "gs://Plume_Finder/experiments/"+experiment_path+"/final_checkpoint_model.ckpt"
