...
from Plume_Finder.utils import get_filesystem, download_product, write_json_to_gcp, read_json_from_gcp
from Plume_Finder.data import aviris
import os
import numpy as np
import rasterio
...
from Plume_Finder.models import mag1c
import torch
...
def download_aviris(name:str, path_targz_base:Optional[str] = None,
    ...
    data = pd.read_csv('gs://Plume_Finder/AVIRIS-NG-Flight-Lines.csv')
    data = data.set_index("Name")

    # Download tar.gz
    if name not in data.index:
        print(f"\t ERROR: {name} not found in AVIRIS NG data index (file: gs://Plume_Finder/AVIRIS-NG-Flight-Lines.csv)")
        return False
